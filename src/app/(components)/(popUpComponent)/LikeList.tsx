...
export default function LikeList(props: LikeListProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await dispatch(
        likeApi.endpoints.getPostLike.initiate({
          postId: props.postId,
          page: currentPage,
        })
      );
  ...
  const getPostLikes = useAppSelector(
    (state: RootState) =>
      state.baseApi.queries[`getPostLike-${props.postId}`]
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        className="fixed bottom-0 left-0 h-2/3 w-full bg-white z-50 rounded-t-2xl shadow-[0px_0px_8px_0px_#1a202c]"
      ...
      >
        ...
        >
          {getPostLikes
            ? getPostLikes.data.length > 0
              ? getPostLikes.data.map((list, index) => (
                  <div
