...
function NextPageComponent(props: PostCreateProps) {
  const { audioFile, audioDuration, audioWaveData, toggleNextPageVisibility } =
    props;
      ...
      <div className="px-6 py-4 h-full relative" style={{ minHeight: '80dvh' }}>
        <div className="mb-4">
          <div
            ...
            onClick={() => handleBackFromNextPage()}
          >
          ...
          <div className="mt-[-5px] flex justify-between items-center">
            ...
            <div
              className={`px-5 w-1/2 rounded-md md:w-auto cursor-pointer ${
                activeTab === 'postToFeed' ? 'bg-red-500' : 'bg-gray-300'
              }`}
              onClick={() => handleTabClick('postToFeed')}
            >
              <button className="w-full text-white font-semibold py-2 px-4 rounded-tl-sm rounded-bl-sm transition duration-300 ease-in-out transform hover:scale-105">
                Post to feed
              </button>
            ...
            <div
              className={`px-5 w-1/2 ml-2 rounded-md md:w-auto cursor-pointer ${
                activeTab === 'postAStory' ? 'bg-red-500' : 'bg-gray-300'
              }`}
              onClick={() => handleTabClick('postAStory')}
            >
        ...
        {activeTab === 'postAStory' ? (
          <PostAStory
            audioFile={props.audioFile}
            audioDuration={props.audioDuration}
            audioWaveData={props.audioWaveData}
          />
        ) : null}
        {activeTab === 'postToFeed' ? (
          <PostToFeed
            audioFile={props.audioFile}
            audioDuration={props.audioDuration}
            audioWaveData={props.audioWaveData}
