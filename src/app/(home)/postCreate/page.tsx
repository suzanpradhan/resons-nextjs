...
import { MagnifyingGlass, Microphone, Pause, Play, StopCircle } from 'phosphor-react';
import { useEffect, useRef, useState } from 'react';
import WaveSurfer from 'wavesurfer.js';
import RecordPlugin from 'wavesurfer.js/dist/plugins/record';
import NextPageComponent from './NextPageComponent'; // Update the import path accordingly
...
const UserPostCreatePage = () => {
  ...
  const handleFileChange = (event: any) => {
    ...
    audioElement.addEventListener("loadedmetadata", () => {
      ...
      console.log("Audio duration (minutes):", durationInMinutes);
    ...
    <div className='overflow-y-auto pt-11 pb-[60px]' style={{ height: '100vh' }}>
      ...
      <div style={{ display: isNextPageVisible ? 'none' : 'block' }}
        ...
        <div className="flex justify-between items-center gap-2">
          <div className={`px-5 w-1/2 rounded-md md:w-auto cursor-pointer ${activeTab === 'recorder' ? 'bg-red-500' : 'bg-gray-300'}`} onClick={() => handleTabClick('recorder')}>
          ...
          <div className={`px-5 w-1/2 ml-2 rounded-md md:w-auto cursor-pointer ${activeTab === 'upload' ? 'bg-red-500' : 'bg-gray-300'}`} onClick={() => handleTabClick('upload')}>
